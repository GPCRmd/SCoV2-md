...
def get_fplot_data(dyn_id,traj_id):
    ...
    for int_type in int_type_l:
        ...
        fpdir = "/protwis/sites/files/Precomputed/covid19/flare_plot/%s" % int_type
...
def get_variant_impact_param_weights_fitted_ab_escape(filename):
    weights_path_pre="/protwis/sites/files/Precomputed/covid19/variant_impact/param_weights/"
...
def dynanalysis(request,dyn_id,sel_genome_id=None,variantimpact_def=False):
    ...
    for f in d.covidfilesdynamics_set.all():
        ...
        if filetype.is_trajectory:
            ...
            filedata["path"]=fileobj.filepath.replace("/protwis/sites/files/","")
...
#    mut_impact_path="/protwis/sites/files/Precomputed/covid19/variant_impact/mut_impact/dyn_%s.data" % dyn_id
...
def ajax_variant_impact(request):
    if request.is_ajax() and request.POST:
        ...
        var_input_res_path="/protwis/sites/files/Precomputed/covid19/variant_impact/"
...
def download_varimpact(request,dyn_id,traj_id,position,analysis):
    var_input_res_path="/protwis/sites/files/Precomputed/covid19/variant_impact/%s/" % analysis
    if analysis=="rmsf":
...
def compute_rmsf(rmsfStr,rmsfTraj,traj_frame_rg,traj_sel,strideVal):
    ...
    #manage seleciton
    i=0
    struc_path = "/protwis/sites/files/" + rmsfStr
    traj_path = "/protwis/sites/files/" + rmsfTraj
    small_errors=[]
    atom_sel=None
    if traj_sel == "bck":
        atom_sel="alpha"
...
def home(request):    
    if request.is_ajax() :
        context={}
        input_path="/protwis/sites/files/Covid19Data/Data/tree.data"
...
#                    protdict["mutations"]=list(protdict["mutations"])
        ...
        context={}
        input_path_colors="/protwis/sites/files/Covid19Data/Data/colorscales.data"
